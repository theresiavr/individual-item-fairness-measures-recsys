...
class Collector(object):
    ...
    def eval_batch_collect(
        ...
        if self.register.need('rec.items'):
            ...
            _, sorted_idx = torch.sort(scores_tensor, stable=True, descending=True)
            topk_idx = sorted_idx[:,:max(self.topk)]
        ...
        #get all positive items during train, val, and test, for all users. size = n_user_trainval + n_user_trainval + n_user_test x n_items
        if self.register.need('data.positive_ui'):
            positive_ui = torch.zeros_like(scores_tensor)
            positive_ui[positive_u.long(), positive_i.long()] = 1
            self.data_struct.update_tensor('data.positive_ui', positive_ui)
