...
class Pop(GeneralRecommender):
    ...
    def calculate_loss(self, interaction):
        # interaction = interaction[interaction.label==1]
        item = interaction[self.ITEM_ID]
        
        #self.item_cnt[item, :] = self.item_cnt[item, :] + 1
        new_item, count = torch.unique(item, return_counts=True)
        self.item_cnt[new_item, :]  = self.item_cnt[new_item, :] + count.reshape(-1,1)
